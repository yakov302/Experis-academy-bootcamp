...
namespace cpp
{
    ...
    Queue<T>::Queue(size_t a_capacity)
        : m_capacity(a_capacity), m_size(), m_in(), m_out(), m_array(new T[m_capacity])
    {
    }

    template <typename T>
    Queue<T>::Queue(Queue const &source)
        : m_capacity(source.m_capacity), m_size(source.m_size), m_in(source.m_size), m_out(), m_array(new T[m_capacity])
    {
        size_t count = 0;
        size_t j = 0;
