...
insert the last member in the array into pointer "_item" and decrease the number of elements(m_nItems) by 1. If the amount of empty members in the array is greater than the array increases size "m_blockSize"*2 and if we decrease one "increases size" "m_blockSize" the size will still be less than the original size(m_originalSize), we will reduce the memory allocation of the array in one increases size (m_originalSize) 
...
output parameter:
ERR_NOT_INITIALIZED: if the *_vecto pointer or the *_item poinret  is equal to NUUL or if the memory realloction failed.
...
Inserts into _item variable the member in the _index place in the array.
...
Inserts into the member in the _index place in the array received frome the _item variable .

input parameters:
*_vector: pointer to the structure.
_index: The place in the array where the value from variable "_item" will by enter.
*_item: Contains the value that will be enter into the member in the _index place in the array.

output parameter:
ERR_NOT_INITIALIZED: if the *_vecto pointer is equal to NUUL.
ERR_WRONG_INDEX: If the _index is greater than the number of elements in the array (m_items)
ERR_OK: If _item variable inserts into the array in the _index place successful*/
