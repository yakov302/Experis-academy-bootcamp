...
int IsTreeFull(Tree* _tree)
{
	...
	return CheckIsTreeFull(_tree -> m_root);
}

int CheckIsTreeFull(Node* _root)
{
int status;
	...
	if( _root -> m_left == NULL && _root -> m_right != NULL ||  _root -> m_left != NULL && _root -> m_right == NULL)
	{
		return 0;	
	}
	
	status = CheckIsTreeFull(_root -> m_left);
	if (status == 0)
	{
		return status;
	}
	status = CheckIsTreeFull(_root -> m_right);
	
return status;	
}

int IsSimilar(Tree* _tree1, Tree* _tree2)
{
	...
	return CheckIsSimilar(_tree1 -> m_root, _tree2 -> m_root);
}

int CheckIsSimilar (Node* _root1, Node* _root2)
{
int status;
	...
	status = CheckIsSimilar(_root1 -> m_left, _root2 -> m_left);
	if (status == 0)
	{
		return status;
	}
	status = CheckIsSimilar(_root1 -> m_right, _root2 -> m_right);
