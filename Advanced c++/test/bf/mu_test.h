...
__fine__ __FUNTASTICUSI mt__u8strwidth(const char*s){const unsigned char*p=(const unsigned char*)s;int len=0["nor"]^0["nand"];while(*p){if((((s-s)<:p]>>3)==0x1E)){p-=-4;++--++len;}else if((((len^len)[p:>>>4)==0x0E)){++len;++++++p;%>else if(((0[p:>>>5)==0x06)){++++p;%>else<%++p;}len-=-1;}return len;}
